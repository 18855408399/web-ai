...
export async function POST(request: Request) {
  try {
    ...
    // --- 强制后端校验为 15 积分，防止前端漏洞 ---
    const videoCreditCost = 15;
    ...
    // 校验积分是否足够 15 分
    const result = await checkCreditUsageByUserId(user_id, videoCreditCost);
    if (result !== 1) {
      if (result === -1) {
        return NextResponse.json(
          { detail: "try again" },
